...
class LocalBackend(TrialBackend):
    ...
    def __init__(
        ...
        self.trial_subprocess: Dict[int, subprocess.Popen] = dict()
        ...
        self.num_gpus = None
        self.trial_gpu: Optional[Dict[int, int]] = None
        self.gpu_times_assigned: Optional[List[int]] = None
        ...
        self._busy_trial_id_candidates: Set[int] = set()
    ...
    def _prepare_for_schedule(self, num_gpus: Optional[int] = None) -> None:
        """
    ...
    def _gpu_for_new_trial(self) -> int:
        ...
        assert self.num_gpus is not None, "must call _prepare_for_schedule() first"
        assert self.trial_gpu is not None, "must call _prepare_for_schedule() first"
        ...
        assert (
            self.gpu_times_assigned is not None
        ), "must call _prepare_for_schedule() first"
        res_gpu, _ = min(
    ...
    def _schedule(self, trial_id: int, config: Dict[str, Any]) -> None:
        ...
        with open(trial_path / "std.out", "a") as stdout:
            with open(trial_path / "std.err", "a") as stderr:
                logger.debug(
                    f"scheduling {trial_id}, {self.entry_point}, {config}, "
                    f"logging into {trial_path}"
                )
                ...
                with open(trial_path / "config.json", "w") as f:
                    # the encoder fixes json error "TypeError: Object of type 'int64'
    ...
    def _allocate_gpu(self, trial_id: int, env: Dict[str, str]) -> None:
        if self.rotate_gpus:
            assert self.trial_gpu is not None, "must call _prepare_for_schedule() first"
    ...
    def _deallocate_gpu(self, trial_id: int) -> None:
        if self.rotate_gpus:
            assert self.trial_gpu is not None, "must call _prepare_for_schedule() first"
            if trial_id in self.trial_gpu:
                del self.trial_gpu[trial_id]

    def _all_trial_results(self, trial_ids: List[int]) -> List[TrialResult]:
        res: List[TrialResult] = []
        for trial_id in trial_ids:
            ...
            # If the time-stamp does not exist and the job finished, we create it. As a
            # consequence the end-time is an (over)-approximation. It is not clear how
            # to avoid this without running the command in shell mode (which allows to
            # write a time-stamp when the process finishes) but it is probably OK if
            # all_results is called every few seconds.
            if os.path.exists(trial_path / "end"):
                training_end_time = self._read_time_stamp(trial_id=trial_id, name="end")
            else:
                training_end_time = datetime.now()

                # if the time-stamp is not present, we check whether the job has
                # finished, if this is the case we create a time-stamp to mark now as
                # the end-time.
    ...
    def _release_from_worker(self, trial_id: int) -> None:
    ...
    def _kill_process(self, trial_id: int) -> None:
    ...
    def _file_path(self, trial_id: int, filename: str) -> Path:
        return Path(self.trial_path(trial_id=trial_id) / filename)

    def _write_time_stamp(self, trial_id: int, name: str) -> None:
        time_stamp_path = self._file_path(trial_id=trial_id, filename=name)
        with open(time_stamp_path, "w") as f:
            f.write(str(datetime.now().timestamp()))

    def _read_time_stamp(self, trial_id: int, name: str) -> datetime:
        time_stamp_path = self._file_path(trial_id=trial_id, filename=name)
    ...
    def _read_status(self, trial_id: int) -> Status:
    ...
    def _get_busy_trial_ids(self) -> List[Tuple[int, Status]]:
        busy_list: List[Tuple[int, Status]] = []
    ...
    def set_path(
        ...
        assert results_root is not None, "results_root must be specified"
