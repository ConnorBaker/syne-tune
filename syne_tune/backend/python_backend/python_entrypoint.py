...
"""
An entry point that loads a serialized function from ``PythonBackend`` and executes it
with the provided hyperparameter.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tune_function_root", type=str)
    parser.add_argument("--tune_function_hash", type=str)
    ...
    assert file_md5(root / "tune_function.dill") == args.tune_function_hash, (
        "The hash of the tuned function should match the hash obtained when "
        "serializing in Syne Tune."
    )
